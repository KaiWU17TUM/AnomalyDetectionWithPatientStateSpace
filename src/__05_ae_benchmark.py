...
from utils.ClassAE import LSTM_AE_BENCHMARK, LSTM_VAE_BENCHMARK, GuidedLSTM_AE_BENCHMARK, GuidedLSTM_VAE_BENCHMARK
...
if __name__ == '__main__':
    ...
    # model_type = 'GuidedLSTM_AE_BENCHMARK'
    # model_type = 'LSTM_AE_BENCHMARK'
    # model_type = 'GuidedLSTM_VAE_BENCHMARK'
    model_type = 'LSTM_VAE_BENCHMARK'
    ...
    #                                              pid in pid_train])
    # pickle.dump((pid_train, pid_tr, pid_val, pid_test), open('processed-benchmark/pid_splits.p', 'wb'))
    pid_train, pid_tr, pid_val, pid_test = pickle.load(open('processed-benchmark/pid_splits.p', 'rb'))
    ...
    if model_type == 'LSTM_AE_BENCHMARK':
        model = LSTM_AE_BENCHMARK(config).to(device)
    elif model_type == 'LSTM_VAE_BENCHMARK':
        model = LSTM_VAE_BENCHMARK(config).to(device)
    elif model_type == 'GuidedLSTM_AE_BENCHMARK':
        model = GuidedLSTM_AE_BENCHMARK(config).to(device)
    elif model_type == 'GuidedLSTM_VAE_BENCHMARK':
        model = GuidedLSTM_VAE_BENCHMARK(config).to(device)
    else:
        print(f'{model_type} is not supported!')

    model_name = f"{n_emb}hidden-{n_latent}latent-{kl_weight}kl"
